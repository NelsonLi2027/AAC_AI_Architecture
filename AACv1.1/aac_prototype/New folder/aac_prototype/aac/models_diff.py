...
class AACDiffModel:
    ...
    def __init__(self, vocab_size, d_emb=32, d_h=32, decay=0.95, seed=0):
        rng = np.random.default_rng(seed)
        self.params = {
            "Emb": _init(rng, (vocab_size, d_emb), fan_in=d_emb),
            "Whh": _init(rng, (d_h, d_h)),
    ...
    def forward_episode(self, episode, train=True):
        ...
        for t, tok in enumerate(tokens):
            ...
            if prev_emb is not None:
                # Same fix as AttentionBaseline: bind (previous token as
                # key) -> (current token as value). The write gate stays
                # a function of the *current context* h_t (not just the
                # raw token pair), so it can in principle learn to write
                # more selectively than "always bind adjacent tokens."
                k = ad.matvec(p["Wk"], prev_emb)
                v = ad.matvec(p["Wv"], emb)
